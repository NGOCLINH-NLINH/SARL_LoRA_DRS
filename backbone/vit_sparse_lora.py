...
from backbone.utils.mlp import SparseMlp
...
class Block(nn.Module):
    ...
    def __init__(self, dim, num_heads, mlp_ratio=4., qkv_bias=False, qk_scale=None, drop=0., attn_drop=0.,
                 drop_path=0., kw_percent_on=1, norm_layer=nn.LayerNorm, lora_r=8, num_tasks=10):
        super().__init__()
        self.norm1 = norm_layer(dim)
        self.attn = Attention_LoRA(
            dim, num_heads=num_heads, qkv_bias=qkv_bias, qk_scale=qk_scale,
            attn_drop=attn_drop, proj_drop=drop, lora_r=lora_r, num_tasks=num_tasks)
        # NOTE: drop path for stochastic depth, we shall see if this is better than dropout here
        self.drop_path = DropPath(drop_path) if drop_path > 0. else nn.Identity()
        self.norm2 = norm_layer(dim)
        mlp_hidden_dim = int(dim * mlp_ratio)
        self.mlp = SparseMlp(in_features=dim, hidden_features=mlp_hidden_dim, kw_percent_on=kw_percent_on, drop=drop)
...
class SparseVitLoRA(nn.Module):
    ...
    def __init__(self, img_size=224, patch_size=16, in_chans=3, num_classes=1000, embed_dim=768, depth=12,
                 ...
                 drop_path_rate=0., norm_layer=nn.LayerNorm, embedding_type="Patch", data_type="", kw_percent_on=1,
                 lora_r=8, num_tasks=10):
        super().__init__()
        ...
        self.blocks = nn.ModuleList([
            Block(
                ...
                drop=drop_rate, attn_drop=attn_drop_rate, drop_path=dpr[i], norm_layer=norm_layer, kw_percent_on=kw_percent_on,
                lora_r=lora_r, num_tasks=num_tasks)
...
def vitsmall_sparse_lora(nclasses: int, kw_percent_on=1, p_size=4, embed_type=None, depth=12, lora_r=8, num_tasks=10):
    return SparseVitLoRA(img_size=224, patch_size=16, num_classes=nclasses,
                           embed_dim=384, depth=12, num_heads=6, mlp_ratio=4.0, drop_rate=0.1,
                     kw_percent_on=kw_percent_on, lora_r=lora_r, num_tasks=num_tasks)
