        ...
        for n, layer in enumerate(self._layers):
            ...
            if self.apply_heterogeneous_dropout and not disable_dropout and self.layerwise_dropout[n]:
                if self.training and self._keep_probs:
                    ...
                    heterogeneous_mask = torch.zeros(out.shape[1]).cuda()
                    ...
                    rand_vals = torch.rand(out.shape[1]).cuda()
                    keep_probs = self._keep_probs[f'layer_{n + 1}_classwise'].cuda()
                    classwise_mask = rand_vals < keep_probs[y.cuda()]

                    # If classwise probabilities available, use them instead use heterogeneous dropout
                    layer_mask = torch.where(
                        (keep_probs.sum(1)[y.cuda()] > 0)[:, None].repeat(1, heterogeneous_mask.shape[1]),
                        classwise_mask,
                        heterogeneous_mask
                    )
