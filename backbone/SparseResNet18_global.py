...
class SparseBasicBlock(nn.Module):
    ...
    def __init__(self, in_planes: int, planes: int, stride: int=1, kw_percent_on=0.1, kw_criterion='sum', kw_relu=False, apply_kw=True) -> None:
        """
...
class SparseResNet(nn.Module):
    ...
    def __init__(self, block: SparseBasicBlock, num_blocks: List[int],
                 num_classes: int, nf: int, kw_percent_on=0.1, kw_criterion='sum', kw_relu=False, apply_kw=True) -> None:
        """
        ...
        self.layer1 = self._make_layer(block, nf * 1, num_blocks[0], stride=1, kw_percent_on=kw_percent_on[0], kw_criterion=kw_criterion, kw_relu=kw_relu, apply_kw=apply_kw[0])
        self.layer2 = self._make_layer(block, nf * 2, num_blocks[1], stride=2, kw_percent_on=kw_percent_on[1], kw_criterion=kw_criterion, kw_relu=kw_relu, apply_kw=apply_kw[1])
        self.layer3 = self._make_layer(block, nf * 4, num_blocks[2], stride=2, kw_percent_on=kw_percent_on[2], kw_criterion=kw_criterion, kw_relu=kw_relu, apply_kw=apply_kw[2])
        self.layer4 = self._make_layer(block, nf * 8, num_blocks[3], stride=2, kw_percent_on=kw_percent_on[3], kw_criterion=kw_criterion, kw_relu=kw_relu, apply_kw=apply_kw[3])
    ...
    def _make_layer(self, block: SparseBasicBlock, planes: int,
                    num_blocks: int, stride: int, kw_percent_on: float, kw_criterion: str, kw_relu:bool, apply_kw) -> nn.Module:
        """
        ...
        for stride in strides:
            layers.append(block(self.in_planes, planes, stride, kw_percent_on, kw_criterion, kw_relu, apply_kw))
...
def sparse_resnet18(nclasses: int, nf: int=64, kw_percent_on=0.1, kw_criterion='sum', kw_relu=False, apply_kw=(1, 1, 1, 1)) -> SparseResNet:
    """
    ...
    """
    return SparseResNet(SparseBasicBlock, [2, 2, 2, 2], nclasses, nf, kw_percent_on, kw_criterion, kw_relu, apply_kw)
